...
int main()
{
   ...
   for (int count = 0; count < N_DIVISIONS; count++)
   {
       ...
       cin >> bud;
       auxOffices[count].addBudget(bud, count, 1);
       cout << "Division " << (count + 1) << "'s auxiliary office: ";
       cin >> bud2;
       auxOffices[count].addBudget(bud2, count, 2);
    }
    ...
    for (int count = 0; count < N_DIVISIONS; count++)
    {
       cout << "\tDivision: " << (count + 1) << "\t\t\t$ ";
       cout << setw(7);
       cout << divisions[count].getDivBudget(count) << endl;
       cout << "\tAuxiliary Office of Division " << (count+1);
       cout << "\t$  ";
       cout << auxOffices[count].getDivBudget(count) << endl;
    }
